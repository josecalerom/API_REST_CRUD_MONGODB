...
router.post("/", async (req, res) => {
  ...
  if (!title || !author || !genre || !publication_date || !original_language) {
    return res.status(400).json({
      message:
        "Los campos 'title', 'author', 'genre', 'publication_date' y 'original_language' son obligatorios.",
    });
  }
...
router.patch("/:id", getBook, async (req, res) => {
  if (!req.body.title && !req.body.author && !req.body.genre && !req.body.publication_date && !req.body.original_language) {
    return res.status(400).json({
      message:
        "Al menos uno de los campos ('title', 'author', 'genre', 'publication_date' y 'original_language') es obligatorio.",
    });
  }
